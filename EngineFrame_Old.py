...
from parapy.geom  import (
    ...
    XOY, rotate, translate, FusedSolid, TranslatedShape, Vector
)
...
class EngineFrame(GeomBase):
    ...
    # ------------------------------------------------------------------
    # Material properties and blade-off containment requirements
    # ------------------------------------------------------------------
    ...
    #: [-]  Target fractional containment margin (> 0 means safe)
    ...
    @Attribute
    def sheet_thickness_required(self) -> float:
        """
        Sheet thickness [m] to achieve containment_margin.
        t = E_k * SF * (1 + margin) / (sigma_avg * eps_f * A_inner)
        """
        return (self.kinetic_energy_blade_off * self.safety_factor * (1.0 + self.containment_margin_target)) / (
                (self.material.yield_stress + self.material.ultimate_tensile_strength) / 2.0 * self.material.fracture_strain * self.mean_area_inner)
    @action
    def update_sheet_thickness_for_containment(self):
        """
        Set sheet_thickness to achieve containment_margin_target.
        Modify containment_margin_target in the GUI, then click this action.
        """
    ...
    def nozzle_duct(self):
        ...
        """
        return Nozzle(
            x_offset                = self.length_casing + self.inlet_length,
...
if __name__ == "__main__":
    ...
    gui.display(frame, autodraw=True)
