...
merge():
  MeagenParser.merge(stages, meagen_rows) writes n_blades, the metric chords
...
class MeagenParser:
    ...
    def parse(filepath):
        ...
        with open(filepath, 'r') as fh:
            text = fh.read()
        return MeagenParser._extract_rows(text)
    ...
    def parse_structure(filepath):
        ...
        with open(filepath, 'r') as fh:
            text = fh.read()
        return MeagenParser._extract_structure(text)
    ...
    def merge(stages, meagen_rows, row_order=None):
        ...
        meagen_rows : list[dict]
            Output of MeagenParser.parse(), in physical file order.
        ...
        if len(flat) != len(meagen_rows):
            raise ValueError(
                f"MeagenParser.merge: {len(flat)} rows in stages but "
        ...
        for row, mrow in zip(flat, meagen_rows):
            ...
            row['chords'] = MeagenParser._fit(mrow['chords'], n_sec)
            row['pitch_angles'] = MeagenParser._fit(mrow['pitch_angles'], n_sec)
    ...
    def _extract_rows(text):
        ...
        for bi, start in enumerate(positions):
            ...
            rows.append(MeagenParser._parse_row_block(text[start:end]))
        return rows
    ...
    def _parse_row_block(block):
        ...
        pitch_angles = [MeagenParser._stagger(u, d)
...
if __name__ == '__main__':
    ...
    path = base_dir / "Multall" / "DesignExample" / "stagen.dat"

    nows, n_sec = MeagenParser.parse_structure(path)
    rows = MeagenParser.parse(path)
