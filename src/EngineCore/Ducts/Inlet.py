...
class Inlet(Duct):
    ...
    #: [deg]  Inclination of the inlet outer (cowl) wall to the engine axis.
    #: 0 = cylindrical cowl; >0 = cowl flares outward toward the outlet.
    outer_wall_angle_deg: float = Input(0.0)
    ...
    @Attribute
    def outer_wall_axial_run(self) -> float:
        """Axial length of the straight outer cowl wall (cowl tangent -> outlet) [m]."""
        return {
            "curved":    self.length * (1.0 - self.a_ext_ratio),
            "bellmouth": self.length,
            "polygonal": self.length,
        }[self.lip_profile_type]

    @Attribute
    def r_inlet_outer(self) -> float:
        """Outer (cowl) radius at the inlet face [m], derived so the straight outer
        cowl wall sits at outer_wall_angle_deg to the axis (0 -> cylindrical cowl,
        r_inlet_outer == r_outlet_outer). Overrides Duct.r_inlet_outer."""
        return self.r_outlet_outer - math.tan(math.radians(self.outer_wall_angle_deg)) * self.outer_wall_axial_run
    ...
    def b_ext(self) -> float:
        ...
        return max(
            self.lip_radius_ratio * self.r_inlet_outer,
            0.03 * self.r_inlet_outer
        )
    ...
    def b_int(self) -> float:
        ...
        return max(
            self.lip_radius_ratio * self.r_inlet_outer * 0.7,
            0.02 * self.r_inlet_outer
        )
