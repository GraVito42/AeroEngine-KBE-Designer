...
Geometry:
  ...
  Convergent-Divergent:
    6-point polygon — inlet → throat at x_throat → outlet.
    Throat inner/outer radii and axial position are settable @Inputs.
...
class Nozzle(Duct):
    ...
    @Input
    def r_throat_outer(self) -> float:
        """Outer radius at throat [m]. Default: mean of inlet/outlet wall thicknesses."""
        wall_mean = 0.5 * (self.wall_thickness_inlet + self.wall_thickness_outlet)
        return self.r_throat_inner + wall_mean
    ...
    # ------------------------------------------------------------------
    # Override is_choked — C-D uses physical throat area
    # ------------------------------------------------------------------

    def is_choked(self) -> bool:
        """
        Convergent nozzle: inherits Duct logic (A_min ≤ A*).
        C-D nozzle: choked when the geometric throat area ≤ A*.
        When choked in C-D mode the throat is at Mach 1.0 and the
        divergent section accelerates flow to the design Mach_out.
        """
        if not self.is_convergent_divergent:
            return super().is_choked()
        ...
        return self.area_throat <= sonic_station.area
    ...
    def thrust_coefficient(self) -> float:
        ...
        """
        gamma       = self.station_in.gamma
        r_gas       = self.station_in.r_gas
        ...
        exp_term = 1.0 - p_ratio_amb ** ((gamma - 1.0) / gamma)
        if exp_term <= 0.0:
            return 0.0
        V_ideal = math.sqrt(2.0 * (gamma / (gamma - 1.0)) * r_gas * T_total_in * exp_term)
    ...
    def profile_points(self):
        ...
        C-D: 6 points (inlet → throat → outlet).
        ...
        if self.is_convergent_divergent:
            return [
                ...
                Point(self.x_throat, self.r_throat_outer, 0.0),
...
if __name__ == "__main__":
    ...
    for label, nozzle in [("CONVERGENT", nozzle_conv), ("C-D", nozzle_cd)]:
        ...
        if nozzle.is_convergent_divergent:
            ...
            print(f"    r_throat_outer : {nozzle.r_throat_outer:.4f} m")
