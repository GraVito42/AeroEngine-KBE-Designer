...
# TODO [Architect]: confirm rotate(XOY,'y',90) convention in the assembly.
...
class Duct(EngineComponent):
    ...
    #: [m]      Axial length of the duct (overrides EngineComponent.length)
    length: float = Input(0.1)

    #: [kg/m³]  Material density (overrides EngineComponent.density)
    density: float = Input(2800.0)

    #: [-]      Total-pressure ratio across the duct (default: 2% loss)
    pressure_ratio: float = Input(0.98)
    ...
    def wall_thickness_inlet(self) -> float:
        ...
        return self.r_inlet_outer - self.r_inlet_inner

    @Attribute
    def wall_thickness_outlet(self) -> float:
        """Wall thickness at outlet cross-section [m]."""
        return self.r_outlet_outer - self.r_outlet_inner
    ...
    def r_outlet_outer(self) -> float:
        ...
        return self.r_outlet_inner * 1.1
    ...
    # ------------------------------------------------------------------
    # Geometry — Part 2: body  (shape-agnostic, never overridden)
    # Naming matches EngineComponent.body so that volume/weight are
    # automatically computed by the inherited @Attributes.
    # ------------------------------------------------------------------
